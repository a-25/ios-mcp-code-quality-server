...
export function validateTestFixOptions(options: Partial<TestFixOptions>): { valid: boolean; error?: string } {
...
export function validateLintFixOptions(options: Partial<LintFixOptions>): { valid: boolean; error?: string } {
  if (!options.xcodeproj && !options.xcworkspace) {
    return { valid: false, error: "Either xcodeproj or xcworkspace must be provided" };
  }
  return { valid: true };
